...
class TextReplacementsTableModel(aqt.qt.QAbstractTableModel):
    def __init__(self, model, model_change_callback, hypertts):
    ...
    def _get_headers(self):
        ...
        return [
            i18n.get_text('tp_header_type', lang),
            i18n.get_text('tp_header_pattern', lang),
            i18n.get_text('tp_header_replacement', lang)
        ]
...
class TextProcessing(component_common.ConfigComponentBase):
    def __init__(self, hypertts, model_change_callback):
        ...
        self.textReplacementTableModel = TextReplacementsTableModel(self.model, self.model_change, hypertts)
    ...
    def draw(self, embedded=False): # return scrollarea or content widget
        ...
        groupbox = aqt.qt.QGroupBox(i18n.get_text('tp_group_preview', lang))
        vlayout = aqt.qt.QVBoxLayout()

        vlayout.addWidget(aqt.qt.QLabel(i18n.get_text('tp_label_verify', lang)))

        # first line
        hlayout = aqt.qt.QHBoxLayout()
        label = aqt.qt.QLabel(i18n.get_text('tp_label_enter_sample', lang))
        ...
        # second line
        hlayout = aqt.qt.QHBoxLayout()
        hlayout.addWidget(aqt.qt.QLabel(i18n.get_text('tp_label_transformed', lang)))
        self.sample_text_transformed_label = aqt.qt.QLabel(i18n.get_text('tp_blank_text', lang))
        ...
        # =====================
        groupbox = aqt.qt.QGroupBox(i18n.get_text('tp_group_rules', lang))
        vlayout = aqt.qt.QVBoxLayout()

        self.html_to_text_line_checkbox = aqt.qt.QCheckBox(i18n.get_text('tp_check_html', lang))
        vlayout.addWidget(self.html_to_text_line_checkbox)
        self.strip_brackets_checkbox = aqt.qt.QCheckBox(i18n.get_text('tp_check_brackets', lang))
        vlayout.addWidget(self.strip_brackets_checkbox)
        self.strip_cloze_checkbox = aqt.qt.QCheckBox(i18n.get_text('tp_check_cloze', lang))
        ...
        groupbox = aqt.qt.QGroupBox(i18n.get_text('tp_group_replacement', lang))
        vlayout = aqt.qt.QVBoxLayout()        

        vlayout.addWidget(aqt.qt.QLabel(i18n.get_text('tp_label_add_replace', lang)))
        ...
        hlayout = aqt.qt.QHBoxLayout()
        self.add_replace_simple_button = aqt.qt.QPushButton(i18n.get_text('tp_button_add_simple', lang))
        hlayout.addWidget(self.add_replace_simple_button)
        self.add_replace_regex_button = aqt.qt.QPushButton(i18n.get_text('tp_button_add_regex', lang))
        hlayout.addWidget(self.add_replace_regex_button)
        self.remove_replace_button = aqt.qt.QPushButton(i18n.get_text('tp_button_remove_rule', lang))
    ...
    def update_transformed_text(self):
        ...
        if len(sample_text) == 0:
            label_text = i18n.get_text('tp_blank_text', lang)
        else:
            try:
