...
class ServicesPage(component_common.ConfigComponentBase):
    ...
    def _open_setup_dialog_for_service(self, service):
        ...
        if service.name == "PiperTTS":
            ...
            dlg.exec()
            # The executable_path config should point to the Python interpreter
            # that runs piper_runner.py, NOT the piper engine folder. On Linux
            # use the auto-detected system Python; on Windows the shared engine.
            from .engine_manager import EngineManager
            python_path = EngineManager.get_python_exe()
            if python_path and os.path.exists(python_path):
                self._set_service_config_value_with_ui_sync(service.name, "executable_path", python_path)
            return
