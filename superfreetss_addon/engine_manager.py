...
from .constants import DATA_DIR, SHARED_ENGINE_DIR
...
PYTHON_EXE = os.path.join(SHARED_ENGINE_DIR, 'python.exe')
...
class EngineManager:
    ...
    def ensure_installed(cls, progress_callback: Optional[Callable] = None):
        ...
        try:
            os.makedirs(SHARED_ENGINE_DIR, exist_ok=True)
            
            # 1. Download & Extract Python
            zip_path = os.path.join(SHARED_ENGINE_DIR, 'python.zip')
            if not os.path.exists(PYTHON_EXE):
                ...
                with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                    zip_ref.extractall(SHARED_ENGINE_DIR)
            ...
            # 2. Install Pip
            scripts_dir = os.path.join(SHARED_ENGINE_DIR, 'Scripts')
            pip_exe = os.path.join(scripts_dir, 'pip.exe')
            if not os.path.exists(pip_exe):
                get_pip_path = os.path.join(SHARED_ENGINE_DIR, 'get-pip.py')
            ...
            cls._run_command([PYTHON_EXE, '-m', 'pip', 'install', 'numpy<2.0.0', 'setuptools', 'wheel'])
            ...
            cls._run_command([PYTHON_EXE, '-m', 'pip', 'install', 'soundfile', 'regex', 'rdflib', 'importlib-metadata'])
    ...
    def _configure_python_pth():
        ...
        pth_files = [f for f in os.listdir(SHARED_ENGINE_DIR) if f.endswith('._pth')]
        if pth_files:
            pth_path = os.path.join(SHARED_ENGINE_DIR, pth_files[0])
