...
class Configuration:
    ...
    use_vocabai_api: Optional[bool] = None
    ...
    def enable_stats(self) -> bool:
        ...
        if self.days_since_install() < stats.STATS_DAYS_CUTOFF:
            return True
        return False
...
class ErrorHandling:
    ...
    # Developer / Debug mode for Turbo Download and other internal processes
    ...
    if current_config_schema_version < 2:
        ...
        if constants.CONFIG_BATCH_CONFIG in config:
            for key, value in config[constants.CONFIG_BATCH_CONFIG].items():
                ...
                # Add default for skip_anki_backup during migration
                batch['skip_anki_backup'] = True 
