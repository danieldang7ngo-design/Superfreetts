...
def get_stream_handler():
    ...
    try:
        ...
        if sys.stdout and hasattr(sys.stdout, 'buffer') and not sys.stdout.closed:
            ...
            # Fallback to regular stdout if buffer not available
...
def get_child_logger(name):
    ...
    # If FORCE_DEBUG_MODE is True, always enable logging (useful for quick diagnostics)
    if SILENT_LOGGING_MODE and not FORCE_DEBUG_MODE:
        if hasattr(sys, '_sentry_crash_reporting'):
