        ...
        try:
            ...
            if model_dir not in models:
                ...
                if not os.path.exists(model_path):
                    # Fallback for Piper naming convention
                    # (Piper often has modelname.onnx and modelname.onnx.json)
                    pass
                ...
                log(f"  Model: {model_path} (Exists: {os.path.exists(model_path)})")
                log(f"  Tokens: {tokens_path} (Exists: {os.path.exists(tokens_path)})")
                log(f"  Lexicon: {lexicon_path} (Exists: {os.path.exists(lexicon_path) if lexicon_path else 'N/A'})")
                log(f"  FSTs: {rule_fsts}")
                ...
                log(f"Initializing Sherpa-ONNX engine...")
                try:
