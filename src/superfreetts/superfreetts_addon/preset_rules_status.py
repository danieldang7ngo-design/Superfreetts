...
class RuleActionContext():
    ...
    def __str__(self):
        ...
        # rule status.
        lang = self.status.anki_utils.get_ui_language()
        rule_status = i18n.get_text("status_creating_audio_dots", lang)
...
class PresetRulesStatus():
    ...
    def __str__(self):
        ...
        lang = self.anki_utils.get_ui_language()
        title_text = i18n.get_text("status_preset_rules_progress", lang).format(constants.ADDON_NAME)
        result = f'<b>{title_text}</b><br/>{joined_updates}'
        return result
